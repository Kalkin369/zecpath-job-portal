...
class FinanceService:
    ...
    def get_transactions(self):
        ...
        return PaymentTransaction.objects.select_related(
            "subscription", "subscription__employer", "subscription__plan"
        ).order_by("-created_at")

    def get_subscription_history(self):

        return BillingHistory.objects.select_related("employer", "payment").order_by(
            "-created_at"
        )
    ...
    def get_plan_revenue(self):
        ...
        return (
            ...
            .values("plan_name")
            .annotate(revenue=Sum("amount"), subscriptions=Count("id"))
            .order_by("-revenue")
        )
    ...
    def get_refund_logs(self):
        ...
        return (
            PaymentTransaction.objects.filter(status__in=["refund_pending", "refunded"])
            .select_related(
                "subscription", "subscription__employer", "subscription__plan"
            )
            .order_by("-updated_at")
    ...
    def get_dashboard(self):
        ...
        total_revenue = (
            PaymentTransaction.objects.filter(status="success").aggregate(
                total=Sum("amount")
            )["total"]
            or 0
        )
