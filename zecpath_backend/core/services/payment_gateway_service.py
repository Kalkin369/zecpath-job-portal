...
class PaymentGatewayService:
    ...
    def create_payment_order(
        ...
        logger.info("Payment order created. Order ID: %s,Employer ID: %s",order["id"],employer.id)
    ...
    def verify_payment(
        ...
        if not is_verified:
            ...
            logger.warning(
                "Payment signature verification failed. Order ID: %s",
                razorpay_order_id
            )
        ...
        logger.info(
            "Payment verified successfully. Payment ID: %s",
            razorpay_payment_id
        )
    ...
    def handle_payment_captured(
        ...
        if not payment:
            ...
            logger.warning("Payment transaction not found for gateway payment ID: %s",payment_id)
        ...
        if payment.captured:
            ...
            logger.info("Duplicate payment.captured webhook ignored. Payment ID:%s",payment.gateway_payment_id)
        ...
        subscription.save(
            update_fields=["start_date","end_date","is_active"]
        )
        ...
        logger.info("Payment captured successfully. Payment ID: %s",payment.gateway_payment_id)
    ...
    def handle_payment_failed(
        ...
        if not payment:
            ...
            logger.warning("Payment transaction not found.Gateway Payment ID: %s",payment_id)
    ...
    def handle_refund_processed(
        ...
        logger.info("Refund processed successfully. Payment ID: %s",payment.gateway_payment_id)
    ...
    def get_payment_history(
        ...
        payments = (
            ...
            .filter(
                subscription__employer=employer
            )
            .order_by("-created_at")
        )
    ...
    def process_refund(
        ...
        logger.info("Refund initiated by employer. Payment ID: %s",payment.gateway_payment_id)
