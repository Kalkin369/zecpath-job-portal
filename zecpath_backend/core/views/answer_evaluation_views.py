...
from core.serializers.answer_evaluation_serializer import \
...
@extend_schema(
    ...
    request=inline_serializer(
        name="EvaluateAnswerRequest", fields={"answer_id": serializers.IntegerField()}
    ),
    responses={
...
class EvaluateAnswerAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not answer_id:
            ...
            return Response(
                {"error": "answer_id is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            keywords = (
                answer.question.template.expected_keywords
                if answer.question.template
                else []
            )

            keyword_score = AnswerEvaluationService().calculate_keyword_score(
                answer.answer_text, keywords
            )
            ...
            return handle_exception("EvaluateAnswerAPIView", e, "Evaluation failed")
...
class AnswerEvaluationDetailAPIView(APIView):
    ...
    def get(self, request, evaluation_id):
        ...
        try:
            ...
            evaluation = AnswerEvaluation.objects.filter(
                answer__question__session__ai_call__application__job__employer=request.user.employer
