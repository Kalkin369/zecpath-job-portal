...
@extend_schema(
    ...
    description=(
        "Register a new user account. " "Supported roles are Candidate and Employer."
    ),
...
@extend_schema(
    ...
    summary="User Login",
    description=(
        "Authenticate a registered user and return " "JWT access and refresh tokens."
    ),
    request={
        "application/json": {
            "example": {"email": "candidate@example.com", "password": "Password@123"}
        }
    },
    responses={
        200: OpenApiResponse(description="Login successful."),
        401: OpenApiResponse(description="Invalid credentials."),
        403: OpenApiResponse(description="Account blocked or not verified."),
    },
)
class LoginAPI(APIView):
    ...
    def post(self, request):
        ...
        if not user.is_staff and not user.is_verified:
            return Response({"error": "Please verify your account first."}, status=403)
...
class RefreshAPI(APIView):
    ...
    def post(self, request):
        ...
        if not refresh_token:
            return Response(
                {"status": "fail", "message": "Refresh token required"},
        ...
        try:
            ...
            return success_response({"access": access_token}, "Token refreshed")
