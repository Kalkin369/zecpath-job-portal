...
class ApplicationViewSet(BaseViewSet):
    ...
    def perform_create(self, serializer):
        ...
        application = serializer.save(
            candidate=candidate, resume=file, ats_score=score_data["final_score"]
        )
    ...
    def job_applicants(self, request, job_id=None):
        ...
        applications = self.queryset.filter(job_id=job_id).order_by("-ats_score")
    ...
    @action(
        detail=False, methods=["get"], url_path=r"job/(?P<job_id>[^/.]+)/status-summary"
    )
    def status_summary(self, request, job_id=None):
        ...
        summary = service.get_job_status_summary(request.user.employer, job_id)
    ...
    def download_resume(self, request, pk=None):
        ...
        url = S3Service.generate_presigned_url(application.resume.name)
