...
from core.views.admin_finance_views import (
    DailyRevenueAPIView,
...
from core.views.ai_integration_views import (
...
from core.views.answer_evaluation_views import (
    AnswerEvaluationDetailAPIView,
...
from core.views.auth_views import LoginAPI, RefreshAPI, SignupAPI
...
from core.views.interview_schedule_workflow_views import (
...
from core.views.security_log_views import (
    SecurityLogViewSet,
    SecurityTestAPIView
)
...
router.register(
    "employers", EmployerViewSet, basename="employer"
)
router.register(
    "candidates", CandidateViewSet, basename="candidate"
)
router.register(
    "jobs", JobViewSet
)
...
router.register(
    "saved-jobs", SavedJobViewSet, basename="saved-jobs"
)
router.register(
    "admin/employers", AdminEmployerViewSet, basename="admin-employers"
)
router.register(
    "admin/users", AdminUserViewSet, basename="admin-users"
)
router.register(
    "admin/jobs", AdminJobViewSet, basename="admin-jobs"
)
router.register(
    "notification-logs", NotificationLogViewSet, basename="notification-logs"
)
router.register(
    "ai-calls", AICallViewSet, basename="ai-calls"
)
router.register(
    "ai-sessions", AIInterviewSessionViewSet, basename="ai-sessions"
)
router.register(
    "ai-questions", AIQuestionViewSet, basename="ai-questions"
)
router.register(
    "ai-answers", AIAnswerViewSet, basename="ai-answers"
)
router.register(
    "call-logs", CallLogViewSet, basename="call-logs"
)
router.register(
    "interview-schedules", InterviewScheduleViewSet, basename="interview-schedule"
)
router.register(
    "availability-slots", AvailabilitySlotViewSet, basename="availability-slot"
)
router.register(
    "interview-reminders", InterviewReminderViewSet, basename="interview-reminders"
)
router.register(
    "candidate-reports", CandidateReportViewSet, basename="candidate-reports"
)
router.register(
    "security-logs", SecurityLogViewSet, basename="security-logs"
)
router.register(
    "audit-trails", AuditTrailViewSet, basename="audit-trails"
)
router.register(
    "error-logs", ErrorLogViewSet, basename="error-logs"
)

router.register(
    "subscription-plans", SubscriptionPlanViewSet, basename="subscription-plans"
)

router.register(
    "user-subscriptions", UserSubscriptionViewSet, basename="user-subscriptions"
)

router.register(
    "payment-transactions", PaymentTransactionViewSet, basename="payment-transactions"
)
...
urlpatterns = [
    ...
    path(
        ...
        AnswerEvaluationDetailAPIView.as_view(),
        name="evaluation-detail",
    ),
    path("schedule-interview/", ScheduleInterviewAPIView.as_view()),
    path(
        "reschedule-interview/<int:schedule_id>/", RescheduleInterviewAPIView.as_view()
    ),
    path(
        "generate-report/",
    ...
    path(
        "security-test/",
        SecurityTestAPIView.as_view()),
    path(
        "security-report/",
        SecurityReportAPIView.as_view(),
        name="security-report",
    ),
    path(
        "subscription/access/",
        SubscriptionAccessAPIView.as_view()),
    path(
    ...
    path(
        "admin/finance/daily-revenue/",
        DailyRevenueAPIView.as_view(),
        name="daily-revenue",
    ),
    path(
        "admin/finance/monthly-revenue/",
        MonthlyRevenueAPIView.as_view(),
        name="monthly-revenue",
    ),
    path(
        "admin/finance/plan-revenue/", PlanRevenueAPIView.as_view(), name="plan-revenue"
    ),
    path(
        "admin/finance/payment-failures/",
        PaymentFailureAPIView.as_view(),
